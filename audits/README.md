...
| [AUDIT-MATURE-DEFI-DEEP-HUNT.md](bug-hunts/AUDIT-MATURE-DEFI-DEEP-HUNT.md) | **Second blue-chip wave**: **Compound v3** (single-base lending), **Balancer v3** (Vault rewrite + ERC4626 buffer), **Curve Stableswap-NG** (Vyper), **Kelp rsETH** (LRT). All clean — and the corpus's live-finding **oracle-staleness class recurs on Kelp**, found *known + de-risked* and calibrated, not re-published |
| [AUDIT-DEEP-HUNT-ORACLE-SEAM.md](bug-hunts/AUDIT-DEEP-HUNT-ORACLE-SEAM.md) | **Deep hunts IV** + the through-line: **Aerodrome** (ve(3,3) DEX), **Synthetix v3** (modular perps), **Frax v3** (LSD+lending), **Spark** (Aave-fork + ALM). All clean — the **fork-delta method** + the recurring lesson that the *oracle-integration seam* is where the residual lives, never the core math |
