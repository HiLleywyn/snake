...
- **Bug-hunts** ([`bug-hunts/`](README.md#-bug-hunts--stress-testing-the-one-finding-claim)) — an active fork-diff/invariant hunt that climbs the whole stack, the method holding its discrimination (true negatives on audited code, real highs on buggy contest code, honest non-findings, one disclosed-and-redacted live finding) at every altitude:
  ...
  - **mature-DeFi second wave** ([mature-DeFi deep hunt](bug-hunts/AUDIT-MATURE-DEFI-DEEP-HUNT.md)) — Compound v3 (single-base lending), Balancer v3 (Vault rewrite, adopts v4's transient net-zero), Curve Stableswap-NG (Vyper), Kelp rsETH (LRT); all clean, and the corpus's *live-finding oracle-staleness class recurs on Kelp* — found known + de-risked, recognized/calibrated/private-routed rather than re-published (the whole discipline in one finding).
  - **deep hunts IV — the oracle seam** ([oracle-seam deep hunt](bug-hunts/AUDIT-DEEP-HUNT-ORACLE-SEAM.md)) — Aerodrome (ve(3,3) DEX), Synthetix v3 (modular perps), Frax v3 (LSD+lending), Spark (Aave-fork + ALM); all clean. Names two through-lines: the *fork-delta method* (hunt only the delta from upstream — Spark/Aerodrome) and the strongest empirical law of the deep rounds — *conservation is solved; the oracle-integration seam is the frontier* (the residual lived at the consumer-side oracle read in 3 of 4, restating the corpus's most-reused law).
